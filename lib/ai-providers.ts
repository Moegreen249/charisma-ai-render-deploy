...
export interface AIModel {
  ...
  rpm?: number;
  rpd?: number;
  isCustom?: boolean; // Indicates if this is a custom model added by admin
  addedBy?: string; // User ID who added this custom model
}
...
export const AI_PROVIDERS: AIProviderConfig[] = [
  ...
  },
  {
    ...
    models: [
      // Gemini models (prioritized by recommendation)
      {
        id: "gemini-2.5-flash",
        name: "Gemini 2.5 Flash",
...
export async function getCustomModels(): Promise<AIModel[]> {
  try {
    const customModelsSettings = await prisma.adminSettings.findMany({
      where: {
    ...
    return customModelsSettings.map((setting) => {
...
export async function getAllModelsWithCustom(): Promise<AIModel[]> {
  const staticModels = getAllModels();
  const customModels = await getCustomModels();
  return [...staticModels, ...customModels];
}

// Get available models for a provider including custom ones
export async function getAvailableModelsByProviderWithCustom(
  providerId: AIProvider,
): Promise<AIModel[]> {
  const staticModels = await getAvailableModelsByProvider(providerId);
  const customModels = await getCustomModels();
  const providerCustomModels = customModels.filter(m => m.provider === providerId && m.available);
  
  return [...staticModels, ...providerCustomModels];
}