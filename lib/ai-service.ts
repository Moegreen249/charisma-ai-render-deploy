...
class AIService {
  ...
  private async getAvailableProviders(feature: string): Promise<string[]> {
    ...
    if (availableProviders.length === 0) {
      const allProviders = ['google-gemini', 'openai', 'anthropic', 'vertex-ai'];
  ...
  public async request(aiRequest: AIRequest): Promise<AIResponse> {
    ...
    for (let i = 0; i < availableProviders.length; i++) {
      ...
      try {
        const response = await this.callProvider(providerId, aiRequest);
        ...
        return {
          ...
          response,
          provider: providerId,
          model: aiRequest.model || this.getDefaultModel(providerId),
          responseTime,
          fallbackUsed: i > 0,
        };
      } catch (error: any) {
  ...
  private async callProvider(providerId: string, aiRequest: AIRequest): Promise<string> {
    ...
    switch (providerId) {
      case 'google-gemini':
        return await this.callGoogleGemini(model, aiRequest.prompt, temperature);
  ...
  private async callGoogleGemini(model: string, prompt: string, temperature: number): Promise<string> {
    const apiKey = await this.getApiKey('google-gemini');
    if (!apiKey) {
