...
// Available AI providers and models - now managed centrally
const AI_PROVIDERS = {
  'google-gemini': {
    name: 'Google Gemini',
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      defaultProvider: Object.keys(AI_PROVIDERS).find(key => AI_PROVIDERS[key as keyof typeof AI_PROVIDERS].available) || 'google-gemini'
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!AI_PROVIDERS[provider as keyof typeof AI_PROVIDERS]?.available) {
      return NextResponse.json(
        { error: `Provider ${provider} is not available. Please check your API key configuration.` },
        { status: 503 }
      );
    }
